...
# def straight_line_planner(ax, ay, ds=0.01):
#     cx, cy, cyaw = [], [], []
#     assert len(ax) == len(ay)
#     for i in range(len(ax)-1):
#         distance = ( (ax[i]-ax[i+1])**2 + (ay[i]-ay[i+1])**2 )**0.5
#         n = int(distance / ds)
#         cx.extend(np.linspace(ax[0], ax[1], n).tolist())
#         cy.extend(np.linspace(ay[0], ay[1], n).tolist())
#         yaw = math.atan2(ay[1]-ay[0], ax[1]-ax[0])
#         cyaw.extend([yaw for i in range(n)])
#         # print(cx)
#         # assert False
#     return cx, cy, cyaw
...
def main():
    ...
    cx, cy, cyaw, ck, s = cubic_spline_planner.calc_spline_course(
        ax, ay, ds=0.1)
    # cx, cy, cyaw = straight_line_planner(ax, ay, ds=0.0001)
    ...
    state = State(x=3.3828, y=-2.0531, yaw=np.radians(-0.2498), v=0.0)
